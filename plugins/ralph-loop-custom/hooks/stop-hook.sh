...
# Read hook input from stdin with timeout (Windows compatibility fix)
# On Windows, stdin may not close properly, causing 'cat' to hang indefinitely
# Use timeout to prevent blocking - 5 seconds should be more than enough
if command -v timeout &> /dev/null; then
  # GNU timeout available (Linux, Git Bash with coreutils)
  HOOK_INPUT=$(timeout 5 cat 2>/dev/null || echo '{}')
else
  # Fallback: read with bash timeout (works on most systems)
  HOOK_INPUT=""
  while IFS= read -r -t 5 line; do
    HOOK_INPUT="${HOOK_INPUT}${line}"
  done
  # If no input received, use empty JSON
